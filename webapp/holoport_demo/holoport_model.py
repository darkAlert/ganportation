...
class HoloportModel(object):
    ...
    def __init__(self, connector, label=None, path_to_conf='yolo_vibe_lwgan_conf_azure.yaml'):
        # Load config:
        path_to_conf = os.path.join(os.path.dirname(__file__), path_to_conf)
        conf = parse_conf(path_to_conf)
        self.connector.logger.info('Config has been loaded from', path_to_conf)
